    ...
    <section class="chat">
      ...
      <section v-else aria-live="polite">
        <div v-for="message in messages" id="message" :key="message.responseId">
          ...
          <RichComponent
            v-for="(component, component_id) in message.queryResult.fulfillmentMessages"
            :key="component_id"
            :fullwidth="component.carouselSelect !== undefined || component.rbmCarouselRichCard !== undefined || component.payload && component.payload.richContent !== undefined"
          >
            <!-- Text (https://cloud.google.com/dialogflow/docs/reference/rest/v2beta1/projects.agent.intents#Text) -->
            <RichBubble
              v-if="component.text"
              :text="component.text.text[0] || ((translations[lang()] && translations[lang()].noContent) || translations[config.fallback_lang].noContent)"
            />
            ...
            <div v-if="component.rbmText">
              <RichBubble
                :text="component.rbmText.text || ((translations[lang()] && translations[lang()].noContent) || translations[config.fallback_lang].noContent)"
              />
              <div
            ...
            >
              <div
                v-for="(suggestion, suggestion_id) in component.rbmStandaloneRichCard.cardContent.suggestions"
                :key="suggestion_id"
              >
                <RichCardButton
                  v-if="suggestion.reply"
          ...
          <section
            v-if="message.queryResult.webhookPayload && message.queryResult.webhookPayload.google"
          >
            <RichComponent
        ...
        <div v-if="loading" id="message">
          <!-- My message (Loading) -->
          <RichComponent me>
            <RichBubble aria-hidden="true" me loading />
          </RichComponent>

          <!-- Default / Webhook bubble (Loading) -->
          <RichComponent>
            <RichBubble aria-hidden="true" loading />
          </RichComponent>
        </div>
      </section>
    </section>
      ...
      if (response.outputAudio) {
        ...
        for (const component in response.queryResult.fulfillmentMessages) {
          if (response.queryResult.fulfillmentMessages[component].text)
            text += `${response.queryResult.fulfillmentMessages[component].text.text[0]}. `;
          if (
          ...
          if (response.queryResult.fulfillmentMessages[component].rbmText)
            text += `${response.queryResult.fulfillmentMessages[component].rbmText.text}. `;
        }
